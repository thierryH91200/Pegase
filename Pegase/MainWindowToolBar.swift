...
extension MainWindowController {
    ...
    @IBAction func appearanceSelection(_ sender: NSSegmentedControl) {
        let appearance: NSAppearance.Name = sender.selectedSegment == 0 ? .aqua : .vibrantDark
...
//            _ = NSWorkspace.shared.launchApplication(path!)
        ...
        panel.beginSheetModal(for: self.window!) { (result) in
            ...
            if result.rawValue == NSApplication.ModalResponse.OK.rawValue {
                ...
                if let keys = allKey
                {
                    ...
                    self.keyInkeys (keys: keys, dateFormatter: dateFormatter, now: now)
                    
                    let dateTransaction = Localizations.ImportSimplifiee.dateTransaction
                    ...
                    for key in keys
                    {
                        ...
                        let entityTransaction = NSEntityDescription.insertNewObject(forEntityName: "EntityTransactions", into: mainObjectContext) as! EntityTransactions

                        entityTransaction.account = currentAccount
                        
                        entityTransaction.dateCree = Date()
                        entityTransaction.dateModifie = Date()
                        entityTransaction.dateTransaction = dateFormatter.date(from: key[dateTransaction] ?? now)
                        entityTransaction.datePointage = dateFormatter.date(from: key[datePointage] ?? now)
                        ...
                        entityTransaction.paymentMode = entityModePaiement
                        ...
                        entityTransaction.statut = numStatut
                        ...
                        entityTransaction.account?.name = key["compte"] ?? entityPreference.account?.name
                    }
    ...
    func keyInkeys(keys:  [[String : String]], dateFormatter: DateFormatter, now : String) {
    ...
    func createExportString() -> String {
        ...
        for listeTransaction in listeTransactions {
            ...
            for sousOperation in sousOperations {
                ...
                data = dateFormatter.string(from: listeTransaction.dateTransaction!)
                export = "\(quote)\(data)\(quote)\(delimiter)"
