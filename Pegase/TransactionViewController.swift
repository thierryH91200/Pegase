...
final class TransactionViewController: NSViewController {
    ...
    @IBOutlet weak var dateTransaction: TFDatePicker!
...
//    var oldEntitySousOperation : EntitySousOperations?
//    var newEntitySousOperation : EntitySousOperations?
    ...
    var setDateTransaction = Set<Date>()
    ...
    override func viewDidLoad() {
        ...
        dateTransaction.locale = Locale.current
    ...
    func contextSaveEdition() {
        ...
        if edition == false {
            ...
            self.entityTransaction = NSEntityDescription.insertNewObject(forEntityName: "EntityTransactions", into: mainObjectContext) as? EntityTransactions
        ...
        // creation = one operation
        if self.entityTransactions.count == 1 && edition == true {
            
//            let setSousOperations = NSSet(array: subOperations)
            
//            let sub = entityOperations.first?.sousOperations
//            self.entityOperations.first?.removeFromSousOperations(sub!)
            let sub1 = entityTransactions.first?.sousOperations
            print (sub1!)

//            self.entityTransactions.first?.addToSousOperations(addSplitTransaction())
//            self.entityTransactions.first?.addToSousOperations(addSplitTransaction())
//            self.entityTransactions.first?.addToSousOperations(addSplitTransaction())

            let sub = entityTransactions.first?.sousOperations
            print (sub!.count)

        }
    }
    
    func addSplitTransaction() -> EntitySousOperations{
        ...
        entitySousOperation.libelle = String(amount)
        
        self.entityPreference = Preference.shared.getAllDatas()
        entitySousOperation.category = entityPreference?.category
    ...
    @IBAction func saveActions(_ sender: Any) {
        ...
        /// Multiple value
        for oneTransaction in self.entityTransactions {
            
            oneTransaction.dateModifie = Date()
            
            // DatePointage
            if (setCheck_In_Date.count > 1 && date5 != nil) || setCheck_In_Date.count == 1 {
                oneTransaction.datePointage  = datePointage.dateValue.noon
            }
            
            // Date Operation
            if (setDateTransaction.count > 1 && date4 != nil) || setCheck_In_Date.count == 1 {
                oneTransaction.dateTransaction  = dateTransaction.dateValue.noon
            }
            
            // Relevé bancaire
            if (setReleve.count > 1 && textFieldReleveBancaire.stringValue != "") || setReleve.count == 1 {
                oneTransaction.bankStatement  = textFieldReleveBancaire.doubleValue
            }
            ...
            if (setModePaiement.count > 1 && popUpModePaiement.indexOfSelectedItem != 0) || setModePaiement.count == 1 {
                ...
                oneTransaction.paymentMode = entityMode
            }
            ...
            if (setStatut.count > 1 && popUpStatut.indexOfSelectedItem != 0) || setStatut.count == 1 {
                ...
                oneTransaction.statut = statut
            }
            
            // Operation Link
            if (setTransfert.isEmpty == false && popUpTransfert.indexOfSelectedItem != 0)  {
                createOperationLiee(oneTransaction: oneTransaction)
            }
        }
    ...
    func createOperationLiee(oneTransaction: EntityTransactions ) {

        print("Operation Liée")        
        if oneTransaction.operationLiee == nil {
            
            self.entityTransactionsTransfert = NSEntityDescription.insertNewObject(forEntityName: "EntityTransactions", into: mainObjectContext) as? EntityTransactions
            self.entityTransactionsTransfert?.operationLiee = oneTransaction
            oneTransaction.operationLiee = entityTransactionsTransfert
        
        } else {
            entityTransactionsTransfert = oneTransaction.operationLiee
        }
        ...
        entityTransactionsTransfert?.dateModifie   = oneTransaction.dateModifie
        entityTransactionsTransfert?.dateCree      = oneTransaction.dateCree

        
        /// Date operation
        entityTransactionsTransfert?.dateTransaction = oneTransaction.dateTransaction
        
        /// Date Pointage
        entityTransactionsTransfert?.datePointage  = oneTransaction.datePointage
        
        // le modePaiement existe t il ??
        let name = oneTransaction.paymentMode?.name
        let color = oneTransaction.paymentMode?.color
        let uuid = oneTransaction.paymentMode?.uuid
        ...
        entityTransactionsTransfert?.statut        = oneTransaction.statut
        entityTransactionsTransfert?.bankStatement = oneTransaction.bankStatement
        
        let entityPreference = Preference.shared.getAllDatas()

        let setSout = oneTransaction.sousOperations
...
//        entityOperationsTransfert?.removeFromSousOperations(setSout!)
        for sousOperation in subOperations {
            ...
            print("count = ", entityTransactionsTransfert?.sousOperations?.count ?? 0)
        }
