...
extension TransactionViewController: ListeOperationsDelegate {
    ...
    func resetOperation() {
        ...
        self.setDateTransaction.removeAll()
        ...
        self.setDateTransaction.insert(Date())
        ...
        self.dateTransaction.dateValue = Date()
        ...
        self.dateTransaction.allowEmptyDate = false
        self.dateTransaction.showPromptWhenEmpty = false
        self.dateTransaction.referenceDate = Date()
        self.dateTransaction.dateFieldPlaceHolder = Localizations.Operation.Multi
        self.dateTransaction.dateValue = Date()
    ...
    func editionOperations(_ quakes: [EntityTransactions]) {
        ...
        if self.entityTransactions.count > 1 {
            ...
            self.dateTransaction.allowEmptyDate = true
            self.dateTransaction.showPromptWhenEmpty = true
        ...
        for quake in quakes {
            ...
            let dateOperation = quake.dateTransaction ?? Date()
            self.setDateTransaction.insert(dateOperation)
        }
        ...
        if setDateTransaction.count > 1 {
            self.date4 = nil
            dateTransaction.updateControlValue(nil)
        } else {
            dateTransaction.dateValue = setDateTransaction.first!
        }
