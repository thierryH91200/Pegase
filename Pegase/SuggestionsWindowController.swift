...
final class SuggestionsWindowController: NSWindowController {
    var action: Selector?
    var target: Any?
    ...
    func begin(for parentTextField: NSTextField?) {
        ...
        var location = parentTextField?.superview?.convert(parentFrame?.origin ?? CGPoint.zero, to: nil)
        ...
        suggestionWindow?.setFrameTopLeftPoint(location ?? CGPoint.zero)
        layoutSuggestions()
        ...
        localMouseDownEventMonitor = NSEvent.addLocalMonitorForEvents(matching: [NSEvent.EventTypeMask.leftMouseDown, NSEvent.EventTypeMask.rightMouseDown, NSEvent.EventTypeMask.otherMouseDown], handler: {(_ event: NSEvent) -> NSEvent? in
            ...
            if event.window != suggestionWindow {
                if event.window == parentWindow {
                    ...
                    let locationTest: CGPoint? = contentView?.convert(event.locationInWindow, from: nil)
                    let hitView: NSView? = contentView?.hitTest(locationTest ?? CGPoint.zero)
     ...
     */
    func setSuggestions(_ suggestions: [[String: String]]) {
     ...
     */
    func selectedSuggestion() -> [String: String]? {
        ...
        for viewController: NSViewController in viewControllers where selectedView == viewController.view {
            suggestion = viewController.representedObject as? [String: String]
            break
        }
        return suggestion
    }
    ...
    private func layoutSuggestions() {
        ...
        for entry in suggestions {
            ...
            // convert the suggestion enty to a mutable dictionary.
            //This dictionary is bound to the view controller's representedObject.
            // The represented object is what all the subviews are bound to in IB. We must use a mutable dictionary because we may change one of its key values.
    ...
    //Notice that there is no mouseDown: implementation. That is because the user may hold the mouse down and drag into another view.
