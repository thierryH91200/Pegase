...
final class ListTransactions {
    ...
    func getAllDatas(ascending: Bool = true ) -> [EntityTransactions] {
        ...
        fetchRequest.sortDescriptors = [NSSortDescriptor(key: "dateTransaction", ascending: ascending)]
    ...
    func adjustDate () {
        ...
        let diffDate = (entities.first?.dateTransaction!.timeIntervalSinceNow)!
        for entityOperation in entities {
            entityOperation.datePointage  = (entityOperation.datePointage!  - diffDate).noon
            entityOperation.dateTransaction = (entityOperation.dateTransaction! - diffDate).noon
        }
...
struct GroupedMonthOperations {
    ...
    init( month: String, idOperations: [IdOperations]) {
        ...
        self.idOperation = idAllOperation.sorted(by: { $0.entityOperations.dateTransaction!.timeIntervalSince1970 > $1.entityOperations.dateTransaction!.timeIntervalSince1970 })
    }
}
...
//        }
//        self.idOperation = idAllOperation.sorted(by: {Double($0.entityOperations.dateTransaction!.timeIntervalSince1970) > Double($1.entityOperations.dateTransaction!.timeIntervalSince1970)})
//    }
//}
//
